...
- `validate_project_layout` checks component containment, physical overlap, and
  simple front-clearance issues from current `design_model.json`
...
- `rescale_imported_model`, `normalize_imported_wall_alignment`,
  `repair_imported_corner_notch`, `review_model_against_import_source`, and
  `repair_imported_region` patch imported working truth when a designer later
  points out a scale, exterior wall alignment, missing stepped corner, or source
  mismatch
